...
export function useTimelineReply(timelineId: string) {
    const [state, dispatch] = useReducer(replyReducer, initialState);

    const fetchReplies = useCallback(async (page = 0) => {
        try {
            const { data } = await api.get(`/timelines/${timelineId}/replies`, {
                params: {
                    page,
    ...
    const fetchChildReplies = useCallback(async (parentId: number, page = 0) => {
        ...
        try {
            const { data } = await api.get(`/timelines/replies/${parentId}/children`, {
                params: {
                    page,
    ...
    const createReply = useCallback(async (content: string, parentId?: number | null) => {
        ...
        try {
            const url = parentId
                ? `/timelines/${timelineId}/replies/${parentId}`
                : `/timelines/${timelineId}/replies`;
    ...
    }, [timelineId, fetchReplies]);

    const updateReply = useCallback(async (replyId: number, content: string) => {
        dispatch({ type: 'SET_SUBMITTING', payload: true });

        try {
            await api.put(`/timelines/replies/${replyId}`, { content });
    ...
    const deleteReply = useCallback(async (replyId: number) => {
        ...
        try {
            await api.delete(`/timelines/replies/${replyId}`);
